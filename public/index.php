...
<div id="app" v-cloak class="flex h-screen" :style="{ fontSize: fontSize + 'px' }">
    ...
    <div v-if="showSetupWizard" class="fixed inset-0 flex items-center justify-center z-50"
        ...
        <div class="rounded shadow-lg p-6" style="background:#000;border:1px solid #00ff00;width:600px;">
            ...
            <!-- Krok: ssh_config -->
            <div v-if="currentSetupStep === 'ssh_config'">
                <p class="text-xs crt-text mb-4">Skonfiguruj połączenie SSH (opcjonalne).</p>
                <div v-if="!setupSkipConfirm" class="flex flex-col gap-2 mb-4">
                    <input v-model="setupStepData.ssh_host" placeholder="Host (np. 192.168.1.100)"
                           class="crt-input px-2 py-1 text-xs rounded">
                    <input v-model="setupStepData.ssh_user" placeholder="Użytkownik SSH"
                           class="crt-input px-2 py-1 text-xs rounded">
                    <input v-model="setupStepData.ssh_port" placeholder="Port (domyślnie 22)"
                           class="crt-input px-2 py-1 text-xs rounded">
                    <select v-model="setupStepData.ssh_auth_method" class="crt-input px-2 py-1 text-xs rounded">
                        <option value="password">Uwierzytelnianie hasłem</option>
                        <option value="key">Klucz SSH</option>
                    </select>
                    <input v-if="setupStepData.ssh_auth_method === 'password'" v-model="setupStepData.ssh_password"
                           type="password" placeholder="Hasło SSH"
                           class="crt-input px-2 py-1 text-xs rounded">
                    <input v-if="setupStepData.ssh_auth_method === 'key'" v-model="setupStepData.ssh_key_path"
                           placeholder="Ścieżka do klucza (np. /home/user/.ssh/id_rsa)"
                           class="crt-input px-2 py-1 text-xs rounded">
                    <div class="flex gap-2 mt-2">
                        <button @click="proceedStep(false)" class="flex-1 crt-button py-1 text-xs">Dalej</button>
                        <button @click="setupSkipConfirm = true" class="flex-1 crt-button py-1 text-xs"
                                style="border-color:#ff6600;color:#ff6600;">Pomiń</button>
                    </div>
                </div>
                <div v-else class="flex flex-col gap-2 mb-4">
                    ...
                    <div class="flex gap-2">
                        <button @click="proceedStep(true)" class="flex-1 crt-button py-1 text-xs"
                                style="border-color:#ff0000;color:#ff0000;">Rozumiem, pomiń</button>
                        <button @click="setupSkipConfirm = false" class="flex-1 crt-button py-1 text-xs">Wróć</button>
                    </div>
                </div>
            </div>

            <!-- Krok: local_directories -->
            <div v-if="currentSetupStep === 'local_directories'">
                <p class="text-xs crt-text mb-4">Dodaj katalog z logami (opcjonalne).</p>
                <div v-if="!setupSkipConfirm">
                    <input v-model="setupStepData.path" placeholder="Ścieżka katalogu (np. /var/log)"
                           class="crt-input px-2 py-1 text-xs rounded w-full mb-4">
                    <div class="flex gap-2">
                        <button @click="proceedStep(false)" class="flex-1 crt-button py-1 text-xs">Dalej</button>
                        <button @click="setupSkipConfirm = true" class="flex-1 crt-button py-1 text-xs"
                                style="border-color:#ff6600;color:#ff6600;">Pomiń</button>
            ...
            <!-- Krok: finalize -->
            <div v-if="currentSetupStep === 'finalize'">
                <p class="text-xs crt-text mb-4">Konfiguracja jest gotowa do zakończenia.</p>
                <button @click="proceedStep(false)" class="w-full crt-button py-1 text-xs">Zakończ konfigurację</button>
            </div>
        </div>
    </div>
    ...
    <aside style="width:350px;min-width:350px;background:#000;border-right:1px solid #00ff00;" class="flex flex-col">
        ...
        <!-- Directory selector -->
        <div v-if="defaultDirectories.length || directories.length" class="px-3 py-2" style="border-bottom:1px solid #00ff00;">
